...
- fix(apollo-forest-run): report a descriptive error instead of 'Cannot read properties of undefined (reading value)' when re-indexing a corrupted list. No behavior change - the malformed write is still accepted, it is only reported better when it is hit (vrazuvaev@microsoft.com_msteamsmdb)
...
- fix(apollo-forest-run): reconcile divergent null and missing chunks during diffing (vrazuvaev@microsoft.com_msteamsmdb)
...
- rename conflicting API (pavelglac@gmail.com)
...
- Support "filter" and "filters" argument variants on Apollo "connection" directive (vrazuvaev@microsoft.com_msteamsmdb)
...
- upgrade minimatch 3.1.3 (celiac@microsoft.com)
